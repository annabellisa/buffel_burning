...
# STRUCTURE PLOT V11 (for plotting with hclust object)

str_plot_V11<-function(K,cluster.data,site.data,las.opt,yaxs.loc,col.pal,...){
  
  head(cluster.data,3)
  
  # identify which cluster each individual was assigned to
  ...
  # Plot: 
  par(mar=c(0,3.8,0,0),mgp=c(0,yaxs.loc+1.5,yaxs.loc),xpd=F)
  p1<-barplot(dat.thisrun,cex.axis=1,col=cols.thisrun,space=0,border=cols.thisrun,xaxt="n",las=las.opt,ylab="Probability \nof assignment",cex.lab=1)
  
  # Add lines for individuals only:
  arrows(1:nrow(sdat.thisrun),0,1:nrow(sdat.thisrun),0.995,code=0,lwd=0.05)
  
  # Draw a box:
  arrows(0,1,nrow(sdat.thisrun),1,code=0,lwd=0.8)
  arrows(0,0,nrow(sdat.thisrun),0,code=0,lwd=0.5)
  
  head(sdat.thisrun,3)
  head(cluster.data,3)
  dim(dat.thisrun)
  
} # close structure plot function V11
